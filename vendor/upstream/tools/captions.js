...
export function getCaptionTools(bridgeOptions) {
    return {
        create_caption_track: {
            description: "Create a caption/subtitle track in the active sequence from an imported caption file (e.g., .srt, .vtt)",
            parameters: {
                type: "object",
                properties: {
                    item_id: {
                        type: "string",
                        description: "Node ID or name of the imported caption project item (e.g., an .srt file)",
                    },
                    start_seconds: {
                        type: "number",
                        description: "Offset in seconds from the start of the sequence (default: 0)",
                    },
                    caption_format: {
                        type: "string",
                        description: "Caption format: 'subtitle' (default), '608', '708', 'teletext', 'ebu', 'op42', 'op47'",
                    },
                },
                required: ["item_id"],
            },
            handler: async (args) => {
                ...
                const format = formatMap[args.caption_format || "subtitle"] ||
