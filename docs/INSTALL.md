...
5. The panel connects automatically to the installed localhost bridge on port 17777; **Connect** retries it with one click. There is no token entry, file picker, or pairing step. The installed native runtime provisions a protected per-user key inside Adobe UXP plug-in storage for automatic mutual authentication.
