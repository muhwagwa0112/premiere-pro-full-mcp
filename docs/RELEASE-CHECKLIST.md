...
- [x] `npm run check` passes all 34 Vitest files / 212 tests; the Windows UI agent passes all 43 tests.
- [x] Support and feature registry regeneration/checks pass for the public actions and feature records.
...
- [x] A two-instance relay live check passes: the second MCP instance relays through the leader's port
  and exposes the full tool set with working `premiere_inspect` on both.
...
- [ ] Push `main`, verify pinned CI, create `v0.4.0`, upload all release assets, and mark it latest.
